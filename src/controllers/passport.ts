...
function parseOCRData(detections: EntityAnnotation[] | null | undefined): PassportEntry[] {
    ...
    const dateRegex = new RegExp(`\\d{1,2}\\s*(${months})\\s*\\d{4}|\\d{2}\\s*\\d{2}\\s*\\d{2}`, 'gi');

    let textBlocks: string[] = extractedText[0].split('\n')
        .map(block => block.trim())
        .filter(block => block.length > 0);

    textBlocks = [...textBlocks, ...textBlocks.map(block => block.split('').reverse().join(''))];
    ...
    textBlocks.forEach((block: string, index: number) => {
        ...
        if (dateRegex.test(normalizedBlock)) {
            ...
            const searchRange = 6;
            for (let i = Math.max(0, index - searchRange); i < Math.min(textBlocks.length, index + searchRange); i++) {
    ...
    stampGroups = stampGroups
        .filter((group, index, self) => 
            index === self.findIndex(g => g.date === group.date))
...
export const uploadPassportPage = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        if (!req.file) {
            res.status(400).json({
...
export const updatePassportData = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
